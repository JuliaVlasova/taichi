...
class News
{
  ...
  */
  ...
  public function __construct( $data=array() ) {
    ...
    if ( isset( $data['title'] ) ) $this->title = $data['title']; /*preg_replace ( "/[^\.\,\-\_\'\"\@\?\!\:\$ a-zA-Zа-яА-Я0-9()]/", "", $data['title'] );*/
    if ( isset( $data['summary'] ) ) $this->summary = $data['summary']; /*preg_replace ( "/[^\.\,\-\_\'\"\@\?\!\:\$ a-zA-Zа-яА-Я0-9()]/", "", $data['summary'] ); */
  ...
  *
  * @param int ID статьи
  * @return News|false Объект статьи или false, если запись не найдена или возникли проблемы
  */

  public static function getById( $id ) {
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "SELECT *, UNIX_TIMESTAMP(publicationDate) AS publicationDate FROM news WHERE id = :id";
    $st = $conn->prepare( $sql );
  ...
  */
  ...
  public static function getList( $numRows=1000000, $order="publicationDate DESC" ) {
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "SELECT SQL_CALC_FOUND_ROWS *, UNIX_TIMESTAMP(publicationDate) AS publicationDate FROM news
  ...
  */
  ...
  public function insert() {
    ...
    // Вставляем статью
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO news ( publicationDate, title, summary, content ) VALUES ( FROM_UNIXTIME(:publicationDate), :title, :summary, :content )";
    $st = $conn->prepare ( $sql );
  ...
  */
  ...
  public function update() {
    ...
    // Обновляем статью
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE news SET publicationDate=FROM_UNIXTIME(:publicationDate), title=:title, summary=:summary, content=:content WHERE id = :id";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":publicationDate", $this->publicationDate, PDO::PARAM_INT );
    $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
    $st->bindValue( ":summary", $this->summary, PDO::PARAM_STR );
